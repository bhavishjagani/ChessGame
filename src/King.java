public class King extends Piece {
    ...
    public boolean isValidMove(int y1, int x1, int y2, int x2, Board board) {
        ...
        return ((Math.abs(x2 - x1) == 1 && y2 - y1 == 0) || (Math.abs(y2 - y1) == 1 && x2 - x1 == 0) || (Math.abs(x2 - x1) == 1 && Math.abs(y2 - y1) == 1)) && (!board.board[y2][x2].hasPiece() || board.board[y2][x2].getPiece().getColor() != this.getColor());
    }
